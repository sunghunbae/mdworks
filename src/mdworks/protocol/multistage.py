...
"""Multi-stage Molecular Dynamics"""
...
# pymbar is imported in the openmmtools
# below codes is to suppress pymbar user warnings
# on timeseries and JAX 64 bit use.
...
class MultiStage(SimFileIO):
    def __init__(self, 
        ...
        # below state values to be changed by stages
        ...
        # below state values to be defined by child class (e.g. protocol.Equilibrium)
        self.pressure = None
        self.stages = None
    ...
    def _create_integrator(self) -> None:
        ...
        """
        self.temperature = 10 * unit.kelvin
        self.friction = 1 / unit.picosecond
        self.timestep = 1 * unit.femtosecond
    ...
    def _change_integrator(self, temperature: float, friction: float, timestep: float) -> None:
        ...
        self.integrator.setTemperature(self.temperature)
        self.integrator.setFriction(self.friction)
        self.integrator.setStepSize(self.timestep)
    ...
    def _add_barostat(self, frequency: int = 25) -> None:
        ...
        if not barostat_exists:
            ...
            self.barostat = self.system.addForce(
                MonteCarloBarostat(self.pressure, self.temperature, self.frequency))
    ...
    def _add_state_data_reporter(self, stage: int, steps: int, interval: int, eta: bool = False) -> None:
        ...
        Args:
            ...
            interval (int): reporting interval (steps)
            eta (bool, optional): whether to report remaining simulation time. Defaults to False.
        """
        tag = self.stages[stage]['tag']
        filename = (self.workdir / f"{self.prefix}_{tag}.ene").as_posix()
